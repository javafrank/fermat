...
import org.fermat.fermat_dap_android_sub_app_asset_issuer_community.v2.dialogs.DisconnectDialog;
import org.fermat.fermat_dap_android_sub_app_asset_issuer_community.v2.util.CommonLogger;
...
public class ContactsListFragment
    ...
    private SettingsManager<AssetIssuerSubAppSettings> settingsManager;
    ...
    private ArrayList<ActorIssuer> lstActorIssuer;
    private ActiveActorIdentityInformation identity;
    private AssetIssuerSubAppSettings appSettings;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        try {
            rootView = inflater.inflate(R.layout.dap_issuer_community_v2_connections_list, container, false);
            ...
            adapter = new ContactsListAdapter(getActivity(), lstActorIssuer, appSession, moduleManager);
...
//            swipeRefresh.setColorSchemeColors(Color.BLUE, Color.BLUE);
    ...
    private void showEmpty(boolean show, LinearLayout emptyView) {
                       ...
                       /* if (swipeRefresh != null)
                        ...
                        if (result != null &&
                            ...
                            if (getActivity() != null && adapter != null) {
                                lstActorIssuer = (ArrayList<ActorIssuer>) result[0];
                                adapter.changeDataSet(lstActorIssuer);
                                if (lstActorIssuer.isEmpty()) {
                                    showEmpty(true, emptyView);
                                } else {
...
//                                swipeRefresh.setRefreshing(false);
    ...
    public class BackgroundAsyncTaskList extends
        ...
        protected Void doInBackground(Void... params) {
            try {
                if (identity != null) {
                    List<AssetIssuerActorRecord> con = moduleManager.getAllActorAssetIssuerRegistered(MAX, offset);
                    if (con != null) {
                        int size = con.size();
                        if (size > 0) {
                            for (AssetIssuerActorRecord conta : con) {
                                if (conta.getDapConnectionState() != null) {
                                    if (conta.getDapConnectionState().getCode().equals(ConnectionState.CONNECTED.getCode())) {
                                        try {
                                            //TODO ver que se hara aqui
//                                            moduleManager.connectToActorAssetIssuer(identity, conta);
                                        } catch (Exception e) {
    ...
    private Object getMoreData() {
        List<AssetIssuerActorRecord> dataSet = new ArrayList<>();
        try {
    ...
    public void onItemClickListener(ActorIssuer data, int position) {
        ...
        if (Build.VERSION.SDK_INT < 23) {
            CommonLogger.info(TAG, getResources().getString(R.string.dap_issuer_community_connection_state) +
                    data.getRecord().getDapConnectionState());
            ...
            try {
                ...
                disconnectDialog.setTitle(getResources().getString(R.string.dap_issuer_community_disconnection_request));
                disconnectDialog.setDescription(getResources().getString(R.string.dap_issuer_community_text_disconnect));
                disconnectDialog.setUsername(data.getRecord().getName() + "?");
    ...
    public void onOptionMenuPrepared(Menu menu) {
        ...
        if (searchItem != null) {
            ...
            searchView.setQueryHint(getResources().getString(R.string.dap_issuer_community_description_search));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        try {
            ...
            switch (id) {
                ...
                case 3:
                    try {
                        applicationsHelper.openFermatApp(SubAppsPublicKeys.DAP_FACTORY.getCode());
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    break;
    ...
    private void showDialogHelp() throws CantGetSettingsException, SettingsNotFoundException {
